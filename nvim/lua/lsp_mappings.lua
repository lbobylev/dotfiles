...
M.setup = function(bufnr)
    ...
    vim.keymap.set('n', '<space>h', vim.lsp.buf.hover, { desc = 'Hover documentation', unpack(opts) })
    vim.keymap.set('n', '<space>s', vim.lsp.buf.signature_help, { desc = 'Signature help', unpack(opts) })
    vim.keymap.set('n', '<space>wa', vim.lsp.buf.add_workspace_folder, { desc = 'Add workspace folder', unpack(opts) })
    ...
    vim.keymap.set('n', '<space>D', vim.lsp.buf.type_definition, { desc = 'Go to type definition', unpack(opts) })
    vim.keymap.set('n', '<space>rn', vim.lsp.buf.rename, { desc = 'Rename symbol', unpack(opts) })
    vim.keymap.set('n', '<space>ca', vim.lsp.buf.code_action, { desc = 'Code actions', unpack(opts) })
    vim.keymap.set('n', '<space>f', function() vim.lsp.buf.format { async = true } end,
        { desc = 'Format code', unpack(opts) })
    vim.keymap.set('v', '<space>f', function() vim.lsp.buf.format { async = true } end,
        { desc = 'Format code', unpack(opts) })
end
